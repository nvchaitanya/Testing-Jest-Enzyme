...
class GitUserSearch extends Component {
  constructor(props) {
    super(props);
    this.state = {
      name: "",
  ...
  componentDidMount() {
    ...
    axios.get("https://api.github.com/search/users?q=masai")?.then((res) => {
      if (res) {
        const data = res.data.items;
        this.setState({
          userData: [...data],
    ...
    return (
      <div className="container-fluid jumbotron-fluid backGnd">
        <div className="container">
          <h1 className="text-center px-5 py-2 bg-color text-light display-4 rounded shadow-lg">
          ...
          {!this.state.isLoading && this.state.isSearchData && (
            <SearchTable tabdata={this.state.userData} />
          )}
          {!this.state.isLoading && this.state.isRepoData && (
            <RepoTable repo={this.state.repoData} />
          )}
        </div>
      </div>
    );
